...
const act2str = (act) => {
  ...
  if ("1" <= act.at(-1) && act.at(-1) <= "9") {
    ...
    return [...components.slice(0, -1), tc2tile(bz2tc(components.at(-1)))].join(
      " "
    );
  } else return act;
};
...
const show_cands = () => {
  ...
  document.getElementById("review-log").innerHTML = `CHAGA Reviewer [Step ${window.__ri}] [Load ${window.__reviews_seats.map(fmtLoad).join(" ")}]`;
...
if (!window.__attached) {
  ...
  const next_stp2 = () => {
    ...
    next_stp();
    if (document.getElementById("status1").innerHTML === status1) window.__ri--;
    show_cands();
  };
  const prev_stp2 = () => {
    window.__ri--;
...
const gameId = document.getElementById("ti").children[0].href.split("=").at(-1);
